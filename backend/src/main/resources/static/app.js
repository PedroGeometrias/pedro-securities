...
function bindSearch() {
  ...
  $("#file-form").addEventListener("submit", async (event) => {
    ...
    setBusy(
      true,
      `Hashing ${file.name} with the C backend, then checking its SHA-256 digest…`,
    );
    try {
      const report = await api("/api/investigations/file", {
        method: "POST",
        body: data,
      });
      renderReport(report);
      await loadHistory();
      showActivity(
        `The backend hashed the file with C. Only SHA-256 ${report.indicator.normalized} was sent to intelligence providers.`,
        "success",
      );
    } catch (error) {
  ...
  $("#export-report").addEventListener("click", () => {
    if (state.current)
      window.location.assign(
        `/api/investigations/${encodeURIComponent(state.current.id)}/export`,
      );
  });
}
...
function bindCopyActions() {
  bindCopyButton(
    "#copy-indicator",
    () => state.current?.indicator.normalized,
    "IOC",
  );
  bindCopyButton("#copy-investigation-id", () => state.current?.id, "ID");
  bindCopyButton("#copy-raw-json", () => $("#raw-json").textContent, "JSON");
  bindCopyButton(
    "#copy-integrity-hash",
    () => state.current?.integrityHash,
    "Hash",
  );
}

function bindCopyButton(selector, valueProvider, label) {
  ...
  button.addEventListener("click", async () => {
    ...
    try {
      ...
      showActivity(`Could not copy ${label} to the clipboard.`, "error");
    }
  });
}
...
async function runIndicator(value, forceRefresh) {
  setBusy(
    true,
    forceRefresh
      ? "Refreshing provider evidence…"
      : "Correlating OTX and VirusTotal evidence…",
  );
  try {
    ...
    await loadHistory();
    showActivity(
      report.cachedEvidence
        ? "Investigation completed from the protected short-term provider cache."
        : "Investigation completed and added to the local integrity chain.",
      "success",
    );
  } catch (error) {
    showActivity(error.message, "error");
...
async function loadStatus() {
  try {
    ...
    $("#system-status").innerHTML = capabilities
      .map(
        ([label, enabled]) =>
          `<span class="status-pill ${enabled ? "ok" : "off"}">${escapeHtml(label)}</span>`,
      )
      .join("");
    $("#demo-banner").hidden = !state.status.demoMode;
  } catch (error) {
    $("#system-status").innerHTML =
      '<span class="status-pill off">Status unavailable</span>';
  }
}
...
function renderReport(report) {
  ...
  $("#reason-list").innerHTML = assessment.reasons
    .map(
      (reason) =>
        `<span class="reason-chip">${escapeHtml(humanize(reason))}</span>`,
    )
    .join("");
  ...
  $("#raw-json").textContent = JSON.stringify(
    Object.fromEntries(
      report.providers.map((provider) => [provider.provider, provider.raw]),
    ),
    null,
    2,
  );
  ...
  $("#native-note").textContent = report.nativeCoreUsed
    ? "C core active"
    : "Java fallback used";
...
function renderMetrics(totals, providers) {
  ...
  const cards = [
    [
      ...
      totals.malicious,
      "engine detections",
      totals.malicious ? "alert" : "good",
    ],
    [
      "Suspicious",
      totals.suspicious,
      "engine results",
      totals.suspicious ? "warn" : "good",
    ],
    [
      "OTX pulses",
      totals.pulses,
      "community matches",
      totals.pulses ? "warn" : "good",
    ],
    [
      "Reputation",
      reputation,
      "lowest source score",
      reputation < 0 ? "alert" : "good",
    ],
    [
      "Sources",
      `${successful}/${providers.length}`,
      "usable responses",
      successful === providers.length ? "good" : "warn",
    ],
  ];
    ...
    .map((provider) => {
      ...
      const metrics = provider.provider.includes("OTX")
        ...
        : [
            ...
            ["Undetected", provider.undetectedCount],
            ["Last seen", lastSeen],
          ];
      return `<article class="provider-card panel">
            ...
            <div class="tag-list">${provider.tags
              .slice(0, 10)
...
function renderEvidence(providers) {
  ...
  $("#evidence-body").innerHTML = evidence
    .map((item) => {
...
function bindHistory() {
  ...
  $("#verify-history").addEventListener("click", async () => {
    ...
    try {
      ...
      showActivity(
        `${result.valid ? "Integrity verified." : "Integrity check failed."} ${result.message} Checked ${result.checkedRecords} record(s).`,
    ...
    $$("[data-history-id]").forEach((button) =>
      button.addEventListener("click", async () => {
        try {
          renderReport(
            await api(
              `/api/investigations/${encodeURIComponent(button.dataset.historyId)}`,
            ),
          );
        } catch (error) {
          showActivity(error.message, "error");
  ...
  $("#run-verification").addEventListener("click", async () => {
    ...
    try {
      ...
      if (!envelope.payload || !envelope.signature)
        throw new Error("This file has no signed payload.");
...
function setBusy(busy, message = "") {
  $$(".primary-button, #refresh-result").forEach(
...
function updateFileLabel(file) {
  ...
  $("#drop-zone small").textContent =
    `${formatBytes(file.size)} · ready for SHA-256`;
}
...
async function api(url, options = {}) {
  ...
  if (!response.ok)
    throw new Error(
      body?.message || body || `Request failed with HTTP ${response.status}`,
    );
  return body;
}
